...
def search():
    ...
    #for each element in frontier, store (path cost, store cost for its parent to visit it, the node itself)
    frontier.put((curr.get_cost(board), curr, None))
    nodesExplored = 0
    while not frontier.empty():
        ...
        for node in expand:
            ...
            node.pathCost = node.parent.pathCost + node.get_cost(board)
            #print("pathCost to",node,"is",node.pathCost)
            """
            if node.curr_pos in board.goal_pos:
                print("\n\n\n")
                print("Goal node alerted!")
                print("\n\n\n")
            """
            frontier.put((node.pathCost, node, node.parent))
